...
Valid frontmatter and body content are required for `npm run build` to succeed.
...
Each content file starts with [YAML](https://yaml.org/) frontmatter: metadata in a block wrapped in triple dashes (`---`). Use the templates below so every field we rely on is set explicitly. For validation, optional overrides, special pages (such as [404](/404)), and how the site reads these values, see [YAML frontmatter and content schemas](/about/development#yaml-frontmatter-and-content-schemas).
